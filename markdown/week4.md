...
> 今时今日，可以这样说，神经网络已经造出了真正的智能机器。为什么神经网络有这么大的能力？非线性特征能力，能处理大量数据，能并行计算吗？还是说大道至简
...
> $$
> ${X}^{T}$是所有的样本数据，不同于${{z}^{\left( 2 \right)}}={{\theta }^{\left( 1 \right)}}x$中的$x$,$x$只是一个样本点，${X}^{T}$是所有的样本点数据
